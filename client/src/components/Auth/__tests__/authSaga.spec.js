...
import { login, refreshToken, register } from '../Auth.action';
import authSaga, { loginSaga, refreshTokenSaga, registerSaga } from '../Auth.saga';
...
describe('Auth saga', () => {
  ...
  it('should save user info after called api successfully', async () => {
    const response = { accessToken: 'accessToken' };
    const result = generator.next(response).value;

    expect(result.payload.action.payload).toMatchObject({
